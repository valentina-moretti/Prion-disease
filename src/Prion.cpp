...
void HeatNonLinear::assemble_system()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      ...
      const double spreading_loc = spreading.value(fe_values.quadrature_point(q));
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          cell_matrix(i, j) += fe_values.shape_grad(i, q) * spreading_loc *
        ...
        // ------------------------------------------- (R.2) ------------------------------------------- //
        cell_residual(i) -= fe_values.shape_grad(i, q) * spreading_loc *
...
void HeatNonLinear::solve_linear_system()
{
...
void HeatNonLinear::solve_newton()
{
...
void HeatNonLinear::output(const unsigned int &time_step, const double &time) const
{
