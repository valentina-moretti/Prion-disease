      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          ...
          const double alpha_loc = alpha_loc.value(fe_values.quadrature_point(q));
          const double spreading_loc =
            spreading_loc.value(fe_values.quadrature_point(q));
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) -= spreading_loc.divergence(i,q) * solution_gradient_loc[q] * 
                                        fe_values.shape_grad(i, q) * fe_values.JxW(q);
                
                  // ------------------------------------------- (A.3) ------------------------------------------- // 
                  // Non-linear stiffness matrix, second term.
                  cell_matrix(i, j) -= spreading_loc.divergence(j,q) * solution_loc[q] * 
                                        fe_values.shape_grad(j, q) *
                                        fe_values.shape_grad(i, q) * fe_values.JxW(q);
                    
                  // ------------------------------------------- (A.4) ------------------------------------------- // 
                  cell_matrix(i, j) -= (alpha_loc * ( 1 - 2 * solution_loc[q]) ) *
                                        solution_gradient_loc[q] *
                                        fe_values.shape_grad(i, q) *
                                        fe_values.JxW(q);

                  // ------------------------------------------- (A.5) ------------------------------------------- // 
                  cell_matrix(i, j) -= (alpha_loc * solution_loc[q]) *( 1 - solution_loc[q]) *
                                        fe_values.shape_grad(j, q) *
                                        fe_values.shape_grad(i, q) *
                                        fe_values.JxW(q);

                  // ------------------------------------------- (A.6 -> B transpose) ------------------------------------------- // 
                  // il meno lo si aggiunge -> vd teoria
              ...
              // ------------------------------------------- (R.2) ------------------------------------------- //
              cell_residual(i) -= solution_loc[q] * spreading_loc.divergence(i,q) * 
